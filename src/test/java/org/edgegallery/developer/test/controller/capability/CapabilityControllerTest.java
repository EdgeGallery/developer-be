...
public class CapabilityControllerTest {
    ...
    @Test
    @WithMockUser(roles = "DEVELOPER_ADMIN")
    public void testGetCapabilityByIdSuccess() throws Exception {
        Capability either = new Capability();
        String url = String.format("/mec/developer/v2/capabilities/%s", "4c22f069-e489-47cd-9c3c-e21741c857oo");
        Mockito.when(capabilityService.findById(Mockito.anyString())).thenReturn(either);
        ResultActions actions = mvc
    ...
    @Test
    @WithMockUser(roles = "DEVELOPER_ADMIN")
    public void testGetAllCapabilityByFilterSuccess() throws Exception {
        List<Capability> either = new ArrayList<>();
        String url = String.format("/mec/developer/v2/capabilities/?filterType=%s", "name");
        Mockito.when(capabilityService.findAll()).thenReturn(either);
        ResultActions actions = mvc
            .perform(MockMvcRequestBuilders.get(url).contentType(MediaType.APPLICATION_JSON_UTF8))
            .andExpect(MockMvcResultMatchers.status().isOk());
        Assert.assertEquals(200, actions.andReturn().getResponse().getStatus());
