...
public class ConstraintViolationExceptionToResponseConverter implements
    ...
    public Response convert(SwaggerInvocation swaggerInvocation, ConstraintViolationException e) {
        FormatRespDto error = new FormatRespDto(javax.ws.rs.core.Response.Status.BAD_REQUEST, e.getMessage());
        return Response.createFail(
            new InvocationException(error.getEnumStatus().getStatusCode(), error.getEnumStatus().getReasonPhrase(),
                error.getErrorRespDto()));
    }
