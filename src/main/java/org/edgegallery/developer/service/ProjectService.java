...
public class ProjectService {
	...
	public Either<FormatRespDto, ApplicationProject> createProject(String userId, ApplicationProject project) {
		...
		if (!CollectionUtils.isEmpty(appProjectCapabilities)) {
			for (ApplicationProjectCapability appProjectCapability : appProjectCapabilities) {
				Either<FormatRespDto, ApplicationProjectCapability> appProjectCapabilityResult = projectCapabilityService
						.create(appProjectCapability);
	...
	public Either<FormatRespDto, Boolean> terminateProject(String userId, String projectId, String token) {
		...
		boolean terminateResult = HttpClientUtil.terminateAppInstance(basePath, testConfig.getAppInstanceId(), userId,
				token);
		if (!terminateResult) {
	...
	public File createCsarPkg(String userId, ApplicationProject project, ProjectTestConfig testConfig)
		...
		if (!CollectionUtils.isEmpty(yamlPoList)) {
			...
			if (mepCapability == null || mepCapability.isEmpty()) {
				chartFileCreator.setChartValues("false", "true", namespace, configMapName, imageDomainName,
						imageProject);
			} else {
				chartFileCreator.setChartValues("true", "true", namespace, configMapName, imageDomainName,
						imageProject);
			}
			// stop
	...
	public boolean deployTestConfigToAppLcm(File csar, ApplicationProject project, ProjectTestConfig testConfig,
		...
		boolean instantRes = HttpClientUtil.instantiateApplication(basePath, appInstanceId, userId, token, lcmLog,
				pkgId, host.getMecHost(), inputParams);
	...
	public boolean checkDependency(ApplicationProject project) {
		...
		Type groupType = new TypeToken<List<OpenMepCapabilityGroup>>() {
		}.getType();
		List<OpenMepCapabilityGroup> capabilities = gsonGroup.fromJson(gsonGroup.toJson(project.getCapabilityList()),
				groupType);
		for (OpenMepCapabilityGroup group : capabilities) {
			...
			Type openMepCapabilityType = new TypeToken<List<OpenMepCapability>>() {
			}.getType();
	...
	private Either<FormatRespDto, Boolean> publishApp(JsonObject jsonObject, String token) {
		...
		if (appId != null && packageId != null) {
			ResponseEntity<String> publishRes = AppStoreUtil.publishToAppStore(appId.getAsString(),
					packageId.getAsString(), token);
			if (publishRes == null) {
	...
	public Either<FormatRespDto, Boolean> getWorkStatus(String projectId, String token) {
		...
		String workStatus = HttpClientUtil.getWorkloadStatus(host.getProtocol(), host.getLcmIp(), host.getPort(),
				config.getAppInstanceId(), userId, token);
		LOGGER.info("pod workStatus: {}", workStatus);
		String workEvents = HttpClientUtil.getWorkloadEvents(host.getProtocol(), host.getLcmIp(), host.getPort(),
				config.getAppInstanceId(), userId, token);
	...
	private boolean deleteDeployApp(ProjectTestConfig testConfig, String userId, String token) {
		...
		if (!CollectionUtils.isEmpty(testConfig.getHosts()) && StringUtils.isNotEmpty(appInsId)) {
			...
			if (StringUtils.isNotEmpty(testConfig.getPackageId())) {
				// delete hosts
				boolean deleteHostRes = HttpClientUtil.deleteHost(basePath, userId, token, testConfig.getPackageId(),
						host.getLcmIp());
		...
		try (CloseableHttpClient client = createIgnoreSslHttpClient()) {
			...
			for (ApplicationProject project : projects) {
				...
				if (status.equals(EnumProjectStatus.DEPLOYED) || status.equals(EnumProjectStatus.DEPLOYED_FAILED)
						&& StringUtils.isNotEmpty(accessToken) && timeDiff.intValue() >= 24) {
