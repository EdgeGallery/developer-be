...
public class ActionIterator implements IActionIterator {

    private IActionCollection collection;

    private int index  = 0;

    public ActionIterator(IActionCollection collection){
        this.collection = collection;
    }

    @Override
    public boolean hasNext() {
        return index < collection.getActionList().size();
    }

    @Override
    public IAction nextAction() {
        if(hasNext()){
            return collection.getActionList().get(index++);
        }
        return null;
    }
}
